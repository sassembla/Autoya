...
namespace Portable.Text {
	...
	class ISO2022JPEncoding : MonoEncoding
	{
		...
		{
			return new ISO2022JPEncoder (this, allow_1byte_kana, allow_shift_io).GetByteCountImpl (chars, charCount, true);
		}
	...
	{
		static readonly JISConvert convert = JISConvert.Convert;
		readonly bool allow_1byte_kana, allow_shift_io;
		...
		{
			return GetBytesImpl (chars, charCount, null, 0, refresh);
		}

		unsafe bool IsShifted (byte*bytes)
		{
			return bytes == null ? shifted_in_count : shifted_in_conv;
		}

		unsafe void SetShifted (byte*bytes, bool state)
		{
			if (bytes == null)
		...
		unsafe void SwitchMode (byte* bytes, ref int byteIndex, ref int byteCount, ref ISO2022JPMode cur, ISO2022JPMode next)
		{
			if (cur == next)
				return;
		...
		static readonly char[] full_width_map = {
			'\0', '\u3002', '\u300C', '\u300D', '\u3001', '\u30FB', // to nakaguro
		...
		public unsafe override int GetBytesImpl (char* chars, int charCount, byte* bytes, int byteCount, bool refresh)
		{
			...
			for (int i = charIndex; i < end; i++, charCount--) {
				...
				if (ch >= 0x2010 && ch <= 0x9FA5) {
					...
					HandleFallback (chars, ref i, ref charCount, bytes, ref byteIndex, ref byteCount, this);
	...
	class ISO2022JPDecoder : Decoder
	{
		...
		int ToChar (int value)
		{
			...
			return value + 1 >= convert.Jisx0208ToUnicode.Length || value < 0 ?
				-1 : ((int)(convert.Jisx0208ToUnicode [value])) | (((int)(convert.Jisx0208ToUnicode [value + 1])) << 8);
		}
